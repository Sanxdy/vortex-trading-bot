    ...
    async def _ai_veto(self, symbol: str, strategy: str, ec: dict, regime: str, direction: str = "LONG") -> tuple:
        ...
        if is_grid:
            prompt = (
                ...
                f"EMA20: ${ema20:.4f} ({ema20_slope})  EMA50: ${ema50:.4f} ({ema50_slope})\n"
                f"Price vs EMA20: {'above' if price > ema20 else 'below'}  "
                f"Price vs EMA200: {'above' if ec.get('price_above_200_ema') else 'below'}\n"
                f"Volume Ratio (RVol): {rvol:.2f} ({vol_trend})\n"
                f"BB Upper: ${bb_upper:.4f}  BB Lower: ${bb_lower:.4f}  "
                f"BB Position: {bb_position:.0f}%\n"
                f"Swing High: ${swh:.4f}  Swing Low: ${swl:.4f}\n"
                f"Candles: {candle_str if candle_str else 'N/A'}\n"
                f"Recent trades: {recent_seq}{streak_txt}\n\n"
            ...
            prompt = (
                ...
                f"RSI: {rsi:.0f}\n"
                f"ATR: ${atr_val:.4f}  ATR%: {atr_pct:.2f}%\n"
                f"EMA20: ${ema20:.4f} ({ema20_slope})  EMA50: ${ema50:.4f} ({ema50_slope})\n"
                f"Price vs EMA20: {'above' if price > ema20 else 'below'}  "
                f"Price vs EMA200: {'above' if ec.get('price_above_200_ema') else 'below'}\n"
                f"Volume Ratio (RVol): {rvol:.2f} ({vol_trend})\n"
                f"Swing High: ${swh:.4f}  Swing Low: ${swl:.4f}\n"
                f"BB Position: {bb_position:.0f}%  "
                f"BB Width: ${bb_upper:.4f} / ${bb_lower:.4f}\n"
                f"Candles last 20: {candle_str if candle_str else 'N/A'}\n"
                f"Recent trades: {recent_seq}{streak_txt}\n\n"
                f"Respond with ONLY a JSON object:\n"
                f'{{"action":"ENTER" or "SKIP","confidence":0.0-1.0,"reasoning":"brief explanation"}}'
            )
